...
class GUI:
    def __init__(self):
        ...
        self.window.title("Δημιουργία αρχείου xlsx από τα xml για τις προσλήψεις στην Εργάνη")
    ...
    def get_data(self):
        d_name = filedialog.askdirectory(initialdir="./data/", title="Επιλέξτε τον φάκελο με τα αρχεία xml")

        if d_name == "":
            return

        self.files_dir_name.set(d_name)
        self.data = list()

        for path, dirs, files in walk(d_name):
            for file in files:
                if file[-4:] == ".xml":
                    self.parse_xml_data(os.path.join(path, file))
    ...
    def create_widgets(self):
        ...
        self.l_data = Label(self.f_data, text="Φάκελος με αρχεία xml:")
        self.l_data.grid(column=0, row=0, padx=10, pady=10, sticky=E)

        self.files_dir_name = StringVar()
        self.ntr_files_dir_name = Entry(self.f_data, width=128, state='readonly', textvariable=self.files_dir_name)
        self.ntr_files_dir_name.grid(column=1, row=0, padx=10, pady=10, sticky=W)

        self.btn_get_data = Button(self.f_data, text="Επιλέξτε φάκελο...", command=self.get_data)
