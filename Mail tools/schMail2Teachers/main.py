...
class GUI:
    def __init__(self):
        ...
        self.window.title("Αποστολή μαζικής αλληλογραφίας")
    ...
    def parseXlsxData(self, test):
        ...
        filesCounter = 0
        sendCounter = 0
        for person in self.staff[1:]:
            last_name = person[0]
            first_name = person[1]
            father_name = person[2]
            specialty = person[3]
            excluded = person[5]

            search_term = f'{last_name} {first_name} {father_name} {specialty}'

            if not debugMode:
                recipient_email = person[4]
            else:
            ...
            if not excluded:
                ...
                if len(attachmentsList) == 0:
                    errors += f"Ο εκπαιδευτικός {search_term} δεν έχει αρχεία για αποστολή.\n"
                else:
                    if test:
                        previewText += "-" * 20 + f" {search_term} " + "-" * 20
                        ...
                        try:
                            ...
                            self.stPreview.insert(INSERT,
                                                  f"{sendCounter:2d}) Η αποστολή στον παραλήπτη '{search_term}: {recipient_email}' πέτυχε.\n")
            else:
                excluded_staff += f"Για τον εκπαιδευτικό '{search_term}' υπήρχε εξαίρεση.\n"
    ...
    def nextGoToTabPreview(self):
        ...
        if self.debugChecked.get() and self.debugRecipient.get() == "":
            showwarning(title="Κενό πεδίο...",
                        message="Έχετε ενεργοποιήσει τη δοκιμαστική λειτουργία. "
                                "Για να προχωρήσετε πρέπει υποχρεωτικά να συμπληρώσετε το πεδίο "
                                "'Παραλήπτης για δοκιμαστική αποστολή'.")
            return
