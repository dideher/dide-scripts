    ...
    for row in sheet.iter_rows():
        ...
        for cell in row:
            if cell.value is None:
                ...
                student.append(re.sub(r'([0-9]+)Ο', r'\1ο', text3))
...
class GUI:
    ...
    def get_filename(self):
        ...
        for student in self.students:
            origin_sch = student[0]
            sch_code = student[1]
            student_id = student[2]
            address_street = student[3]
            address_pc = student[4]
            address_area = student[5]
            address = student[6]
            gm_sch = student[7]
            gm_address = student[8]
            gm_coords = student[9]
            gv3_sch = student[10]
            gv3_address = student[11]
            gv3_coords = student[12]
            bm_sch = student[13]
            bm_address = student[14]
            bm_coords = student[15]
            hm_sch = student[16]
            hm_address = student[17]
            hm_coords = student[18]
            assignment_sch = ''
            assignment_address = ''
            assignment_coords = ''

            self.cursor.execute(
                f"INSERT INTO students VALUES ('{origin_sch}', '{sch_code}', '{student_id}', "
                f"'{address_street}', '{address_pc}', '{address_area}', '{address}', "
    ...
    def load_student(self, student_no):
        ...
        current_code = self.students[student_no - 1][1]
        current_id = self.students[student_no - 1][2]

        self.entry_num.set(str(student_no))
        self.cursor.execute(f"SELECT * FROM students "
                            f"WHERE sch_code == '{current_code}' AND student_id == '{current_id}'")
        ...
        hm_address = student[17]
        hm_coords = student[18]
        assignment_sch = student[19]
        assignment_address = student[20]
        assignment_coords = student[21]
    ...
    def address_not_found(self):
        ...
        self.cursor.execute(f"UPDATE students "
                            ...
                            f"WHERE sch_code == '{sch_code}' AND student_id == '{student_id}'")
        self.conn.commit()
    ...
    def create_widgets(self):
        ...
        self.ntr_bm_chool = Entry(self.lf_gecoding_info, width=40, state='readonly', textvariable=self.bm_school)
        self.ntr_bm_chool.grid(column=1, row=2, padx=10, pady=5)
