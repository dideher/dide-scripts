...
class GUI:
    ...
    def getMapDataFilename(self):
        fName = filedialog.askopenfilename(initialdir="./data/",
                                           title="Επιλέξτε το αρχείο με τις περιοχές των Σχολικών Μονάδων",
    ...
    def getStudentsFilename(self):
        ...
        self.studentsCount.set(str(len(self.students) - 1))
    ...
    def find_school(self, address, point):
        if address != "N/A" and address != "Exception.":
            school = searchPointInsidePolygons(point, self.namesList, self.polygonsList)
            if not school:
                schoolText = "N/A"
            else:
                schoolText = ''
                for sch_i, sch in enumerate(school):
                    schoolText += sch

                    if sch_i < len(school) - 1:
                        schoolText += ', '
        else:
            schoolText = "N/A"
    ...
    def run(self):
        ...
        for i in range(len(data)):
            ...
            self.lProgress.configure(
                text="Εκτέλεση κατανομής σε εξέλιξη... ({}/{})".format(i, self.studentsCount.get()))

            address_to_search = self.students[i][self.studentAddrCol]

            if self.GMchecked.get():
                if address_to_search in gmDict:
                    schoolText = gmDict[address_to_search][0]
                    address = gmDict[address_to_search][1]
                    point = gmDict[address_to_search][2]

                    print(f"gmDict: [{address_to_search}] --> ['{schoolText}', '{address}']")
                else:
                    point, address = searchAddressInGoogleMaps(self.myGoogleKey, address_to_search)
                    schoolText = self.find_school(address, point)
                    gmDict[address_to_search] = [schoolText, address, point]
            ...
            if self.GV3checked.get():
                if address_to_search in gv3Dict:
                    schoolText = gv3Dict[address_to_search][0]
                    address = gv3Dict[address_to_search][1]
                    point = gv3Dict[address_to_search][2]

                    print(f"gv3Dict: [{address_to_search}] --> ['{schoolText}', '{address}']")
                else:
                    point, address = searchAddressInGoogleV3(self.myGoogleKey, address_to_search)
                    schoolText = self.find_school(address, point)
                    gv3Dict[address_to_search] = [schoolText, address, point]
            ...
            if self.BMchecked.get():
                if address_to_search in bmDict:
                    schoolText = bmDict[address_to_search][0]
                    address = bmDict[address_to_search][1]
                    point = bmDict[address_to_search][2]

                    print(f"bmDict: [{address_to_search}] --> ['{schoolText}', '{address}']")
                else:
                    point, address = searchAddressInBingMaps(self.myBingKey, address_to_search)
                    schoolText = self.find_school(address, point)
                    bmDict[address_to_search] = [schoolText, address, point]
            ...
            if self.HMchecked.get():
                if address_to_search in hmDict:
                    schoolText = hmDict[address_to_search][0]
                    address = hmDict[address_to_search][1]
                    point = hmDict[address_to_search][2]

                    print(f"hmDict: [{address_to_search}] --> ['{schoolText}', '{address}']")
                else:
                    point, address = searchAddressInHereMaps(self.myHereKey, address_to_search)
                    schoolText = self.find_school(address, point)
                    hmDict[address_to_search] = [schoolText, address, point]
        ...
        while notSaved:
            try:
                ...
                showwarning(title="Αρχείο σε χρήση...",
                            message=f"Παρακαλώ κλείστε το αρχείο '{outputFile}' ώστε να ολοκληρωθεί η αποθήκευση της νέας κατανομής.")
            else:
                notSaved = False
    ...
    def create_widgets(self):
        ...
        self.btnNextGoToStudents = Button(self.fMapData, text="Επόμενο", command=self.nextGoToTabStudents,
                                          state='disabled')
        ...
        self.ntrStudentsFilename = Entry(self.fStudents, width=128, state='readonly',
                                         textvariable=self.studentsFilename)
        ...
        self.btnNextGoToKeysAPIs = Button(self.fStudents, text="Επόμενο", command=self.nextGoToTabKeysAPIs,
                                          state='disabled')
        ...
        self.ckbGoogleMaps = Checkbutton(self.lfAPIsFrame, text="Google Maps", variable=self.GMchecked,
                                         state='disabled')
