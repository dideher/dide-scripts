...
class OpendataClient(object):
    ...
    def get_THK_types(self):
        return self._get_resource('/dictionaries/THK')


    def get_THK_type_label(self, thk_id):
        items = self._get_resource('/dictionaries/THK')['items']
        for item in items:
            if item['uid'] == thk_id:
                return item['label']
    ...
    def submit_decision(self, metadata, pdf, attachments=[], recipients=[]):
        ...
        return requests.post(url=self._get_resource_url("/decisions"), 
              data=data, files=tuple(files), verify=True,
    ...
    def edit_published_decision(self, ada, metadata, pdf=None, 
        ...
        if not pdf and not attachments:
            ...
            response = requests.post(
                ...
                data=metadata_str, headers=headers, verify=True,
                auth=self._create_auth())
        else:
            data = {'metadata': metadata_str}
            files = []
            ...
            response = requests.post(
                url=self._get_resource_url("/decisions/" + ada), 
                data=data, files=files, verify=True,
                auth=self._create_auth())
    ...
    def submit_revocation_request(self, ada, comment):
        ...
        response = requests.post(
            ...
            data=request_str, headers=headers, verify=True, 
            auth=self._create_auth())
        return response
