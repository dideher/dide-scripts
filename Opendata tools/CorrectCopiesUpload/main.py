...
class GUI():
    def __init__(self):
        ...
        self.window.title("Μαζική διόρθωση Συμβάσεων στη Διαύγεια")
    ...
    def getADAfromFilename(self, file):
        return file[file.rindex(" ") + 1:-4]


    def getNamefromFilename(self, file):
        return file[:file.rindex(" ")].strip()


    def editDicisions(self):
        inputDir = self.filesDirName.get()
        outputDir = os.path.join(inputDir, "correctADAs")
        ...
        for entry in os.scandir(inputDir):
            if entry.is_file():
                file = entry.name
                if file[-4:] == ".pdf":
                    self.editDecision(file, inputDir, outputDir)
    ...
    def upload(self):
        ...
        self.btnUpload.configure(state='disabled')
        self.runThread = Thread(target=self.editDicisions)
    ...
    def saveADAfile(self, url, file, outputDir):
        r = requests.get(url, allow_redirects=True)

        open(os.path.join(outputDir, file), 'wb').write(r.content)


    def editDecision(self, file, inputDir, outputDir):
        ada = self.getADAfromFilename(file)

        metadataFromADA = self.client.get_decision(ada)
        for key in self.metadataTemplate:
            if key in metadataFromADA:
                self.metadataTemplate[key] = metadataFromADA[key]
        self.metadataTemplate['correctedCopy'] = True

        # Decision document
        pdf_file = open(os.path.join(inputDir, file), 'rb')

        # Send request
        response = self.client.edit_published_decision(ada, self.metadataTemplate, pdf_file)
        pdf_file.close()
        ...
        if response.status_code == 200:
            ...
            self.saveADAfile(url, file, outputDir)
    ...
    def create_widgets(self):
        ...
        self.btnUpload.grid(column=1, row=1, padx=10, pady=10)

        self.stPreview = ScrolledText(self.fFiles, width=120, height=16)
        self.stPreview.grid(column=0, row=2, columnspan=3, padx=10, pady=10)
