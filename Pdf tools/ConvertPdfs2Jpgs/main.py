...
class GUI():
    def __init__(self):
        ...
        self.window.title("Μετατροπή αρχείων pdf σε jpg")
    ...
    def run(self):
        ...
        for path, dirs, files in walk(inputDirectory):
            for file in files:
                ...
                # 300 is the DPI, for better quality (and larger file) use 400 or 600
                pages = convert_from_path(path + '/' + file, 300)

                if len(pages) == 1:
                    pages[0].save(f'{outputDirectory}/{file[:-4]}.jpg', 'JPEG')
                else:
                    for i in range(len(pages)):
                        pages[i].save(f'{outputDirectory}/{file[:-4]}_{i + 1:04}.jpg', 'JPEG')

                count += 1

        showinfo(title="Ολοκλήρωση επεξεργασίας", message=f'Η μετατοπή των αρχείων pdf σε jpg ολοκληρώθηκε.')


    def create_widgets(self):
        self.fData = Frame(self.window)

        self.lData = Label(self.fData, text="Φάκελος με αρχεία pdf:")
        ...
        self.lOutputDirName = Label(self.fData, text="Φάκελος για αποθήκευση των jpg:")
